...
const GLOBAL_ACCESSES_SET = freeze(
  new Set(GLOBAL_ACCESSES as readonly string[])
);
...
export function assertOrgAccess(name: any): asserts name is OrgAccess {
  if (!isOrgAccess(name)) {
    throw new Error(
      `Access ${name} is not a valid workspace access. Must be one of ${ORG_ACCESSES}`
    );
  }
}
...
const orgRolesEntries = entries(_ORG_ROLES) as [
  OrgRoleName,
  Readonly<OrgAccess[]>
][];

export const ORG_ROLES_BY_ACCESS = freeze(
  orgRolesEntries.reduce((acc, [role, accesses]) => {
