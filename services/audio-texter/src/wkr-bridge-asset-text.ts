import {
	assertEvent,
	getAppQueue,
	getJobQueue,
	VidTextJob,
} from "#common/queue.js";

main();

async function main() {
	const assetAudioMp4Queue = getAppQueue("AssetAudioMp4");

	const vidTextTodoQueue = getJobQueue("VidTextJob");

	const streamGroup = "audio-texter-bgrp";

	for (;;) {
		const entry = await assetAudioMp4Queue.next(streamGroup);
		console.log(entry);

		assertEvent("AssetAudioMp4", entry.data);

		const { orgId, assetId } = entry.data;

		const vidTextTodo: VidTextJob = { type: "VidTextJob", orgId, assetId };
		await vidTextTodoQueue.add(vidTextTodo);

		await assetAudioMp4Queue.ack(streamGroup, entry.id);
	}
}
