...
debug_demon() {
  ...
  /service/node_modules/.bin/nodemon --ignore 'dist/*' --ignore 'test/*' --ignore 'src/*' dist/services/vid-init/src/start
}

# Similar as above, but start the node with --inspect for breakpoint debug
debug_demon_inspect() {
  echo "run mode: DEBUG_DEMON_INSPECT"
  /service/node_modules/.bin/nodemon --inspect --ignore 'dist/*' --ignore 'test/*' --ignore 'src/*' dist/services/vid-init/src/start
}
