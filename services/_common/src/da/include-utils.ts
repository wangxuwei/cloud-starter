...
export interface IncludeProcessResult {
	...
	nested: NestedIncludeConfig[]; // Nested includes for batch loading (hasMany)
}
...
export function processIncludes(
	includes: IncludeObject | undefined,
	...
	for (const [key, spec] of Object.entries(includes)) {
		...
		if (typeof spec === 'object' && !key.startsWith('_')) {
			...
			if (relationship) {
				const alias = relationship.as || `${tableAlias}_${key}`;
				...
				if (relationship.type === 'belongsTo') {
					...
					// For hasMany/hasOne, defer to batch loading with Knex whereIn()
					nested.push({
						relation: key,
						config: relationship,
