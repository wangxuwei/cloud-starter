...
const ERROR = symbolDic("NO_USER_FOUND");
...
// For auth only
const USER_COLUMNS_FOR_AUTH = Object.freeze([
  "id",
  "uuid",
  "username",
  "role",
...
export const USER_COLUMNS_FOR_LOGIN = Object.freeze([
  ...USER_COLUMNS_FOR_AUTH,
  "pwd",
  "psalt",
]);
...
export class UserDao extends BaseDao<User, number, QueryOptions<User>> {
  ...
  async create(utx: UserContext, data: Partial<User>) {
    throw new Err(
      CODE_ERROR,
      "UserDao.create not implemented by design. Use UserDao.createUser"
    );
    return -1; // for TS.
  }
  ...
  parseRecord(user: User) {
    ...
    for (const col of USER_SECURITY_COLUMNS) {
      if ((<any>user)[col] !== undefined) {
        delete (<any>user)[col];
        console.log(
          `CODE ERROR - RECOVERED - UserDao processEntity had to remove '.${col}' for safety. Call completeQueryBuildWithQueryOptions.`
        );
      }
    }
    return user;
  ...
  async createUser(utx: UserContext, data: CreateUserData) {
    ...
    const userId = (
      await query.insert(dataUser).returning(this.idNames as "id")
    )[0].id as number;
  ...
  async setPwd(
    ...
    if (resetPSalt) {
      const { query } = await knexQuery({ utx, tableName: this.table });
      await query
        .update({ psalt: query.client.raw("gen_random_uuid()") })
        .where(userKey);
    }
  ...
  async getUserCredForAuth(
    utx: UserContext,
    key: UserKey
  ): Promise<UserCredForAuth> {
    // Note: can force the return type as per columns
    return this.getUserCred(
      utx,
      USER_COLUMNS_FOR_AUTH,
      key
    ) as Promise<UserCredForAuth>;
  }

  @AccessRequires() // only allow sysCtx
  async getUserCredForLogin(
    utx: UserContext,
    key: UserKey
  ): Promise<UserCredForLogin> {
    // Note: can force the return type as per columns
    return this.getUserCred(
      utx,
      USER_COLUMNS_FOR_LOGIN,
      key
    ) as Promise<UserCredForLogin>;
  }

  @AccessRequires() // only allow sysCtx
  async getUserCredForCreate(
    utx: UserContext,
    key: UserKey
  ): Promise<UserCredForLogin> {
    // Note: can force the return type as per columns
  ...
  private async getUserCred(
    ...
    key: UserKey
  ): Promise<UserCredForAuth | UserCredForLogin> {
    const { query } = await knexQuery({ utx, tableName: this.table });
  ...
  static parseAccess(rawUserObj: {
    role: GlobalRoleName;
    accesses?: string[];
  }): Readonly<{ [key in GlobalAccess]?: true }> {
    ...
    if (accesses) {
      for (const accessModifier of accesses) {
        ...
        if (isAccess(accessName)) {
          ...
          console.log(
            `CODE ERROR - IGNORE - ${accessName} if not a valid GLOBAL_ACCESSES.`
          );
        }
      }
    }
    ...
    return Object.freeze(
      Array.from(userAccesses.values()).reduce((acc, val) => {
        acc[val] = true;
        return acc;
...
function checkUserKey(key: UserKey) {
  ...
  if (keys.length != 1) {
    throw new Err(
      CODE_ERROR,
      `getUserCred key has too many properties ${Object.keys(key).length}`
    );
  }
  ...
  if (!user_keys.includes(keys[0])) {
    throw new Err(
      CODE_ERROR,
      `invalid user key ${keys[0]} should be one of ${USER_KEYS}`
    );
  }
}
//#endregion ---------- /Utils ----------
