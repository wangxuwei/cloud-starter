...
import { getEntityIncludeColumns, getEntityIncludeRelations, getRelationship, IncludeRelationSpec } from './conf.js';
...
export function validateIncludeProcessor(
	...
	const validColumnGroups = new Set(Object.keys(entityIncludeColumns.columnGroups));
	...
	for (const [key, spec] of Object.entries(includeObject)) {
		...
		if (key.startsWith('_')) {
			...
			if (!isValidRelationship && !isValidColumn) {
				throw new Error(
					`Invalid include at path '${currentPath}': '${key}' is not a valid key for entity '${entityKey}'. ` +
					`Valid relationships: ${Array.from(validRelationshipKeys).join(', ')}` + 
					`Valid columns: ${Array.from(validColumns).join(', ')}`
				);
			}
			
			// Recursively validate nested relationships
			if (isValidRelationship && typeof spec === 'object' && !Array.isArray(spec)) {
...
function buildIncludeProcessorOptions(
	...
	// Get valid relationships from current entity (first level) or use passed relations
	const validRelationships = relations || getEntityIncludeRelations(entityKey);
	const currentEntityIncludeRelations =  validRelationships[entityKey];
	...
	for (const [key, spec] of Object.entries(includeObject)) {
		...
		if (key.startsWith('_')) {
			...
			let nestedRelationship = (currentEntityIncludeRelations as any)[key];
			...
			if (isValidRelationship) {
				...
				const nestedOptions = buildIncludeProcessorOptions(relSpec, relKey, relAlias, validNestRelationShip, relPath);
				...
				if (relationType == 'belongsTo'){
					...
					if(Object.keys(nestedOptions.targetRelationColumns).length > 0) {
						for(const nestPath in nestedOptions.targetRelationColumns){
							options.targetRelationColumns[`${path}.${nestPath}`] = nestedOptions.targetRelationColumns[path];
						}
					}
				}
			} else {
