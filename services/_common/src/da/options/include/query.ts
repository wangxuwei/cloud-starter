...
export function buildMainAndRelationColumns(
  ...
  for (const col of Object.entries(options.targetColumns)) {
    query.column(col);
  }
  ...
  for (const [relationKey, relationOptions] of Object.entries(
    ...
    try {
      ...
      if (relationshipDef.type === "belongsTo") {
        ...
        // Use dao.list with CustomQuery to get nested entities
        const records = await nestedDao.list(utx, queryOptions);
